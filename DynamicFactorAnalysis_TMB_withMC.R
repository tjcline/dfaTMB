...
ZmatGen<-function(Data,NumStates,MC=FALSE){
	if(MC){
	    tZ<-matrix(runif(ncol(Data)*NumStates,-1,1),nrow=ncol(Data),ncol=NumStates)
	}else{	
		tZ<-matrix(1,nrow=ncol(Data),ncol=NumStates)
	}
	if(NumStates>1){
...
setParametersAndFactors<-function(obs,NumStates=1,ErrStruc='DE',EstCovar=FALSE,Covars=NULL,indivCovar=FALSE,Dmat=NULL,Dfac=NULL,Rfac=NULL,logsdObs=NULL,logsdObsFac=NULL,cholCorr=NULL,cholFac=NULL,MC=FALSE){
	...
	if(EstCovar){ # If you are estimating covariates this creates Dmat and Dfac based on the data, number of covars
		if(is.null(Dmat) & is.null(Dfac)){ #This checks that you did not supply Dmat or Dfac (Manual covariate paramter entries)
			if(!indivCovar){
				if(MC){
					Dmat<-matrix(runif(ncol(obs)*nrow(Covars),-1,1),ncol=nrow(Covars),nrow=ncol(obs))
				}else{
	...
	if(is.null(logsdObs) & is.null(logsdObsFac) & is.null(cholCorr) & is.null(cholFac)){
		if(ErrStruc=='DE'){
			if(MC){
				logsdObs<-log(rep(runif(1,0,1),ncol(obs)))
			}else{
			...
			cholCorr<-rep(0,ncol(obs)*(ncol(obs)-1)/2)
			if(MC){
				logsdObs<-log(runif(ncol(obs),0,1))
			}else{
				logsdObs<-log(rep(0.5,ncol(obs)))
			}
	...
	parameters <- list(
		...
		covState = diag(NumStates),
  	  	D=Dmat,
  	  	Z=ZmatGen(Data=obs,NumStates=NumStates,MC=MC),
  	  	u=matrix(0,nrow=nrow(obs),ncol=NumStates)
		)

	return(list(data=data,parameters=parameters,Dfac=Dfac,Zfac=Zfac,logsdObsFac=logsdObsFac,cholFac=cholFac))
}
...
runDFA<-function(obs,NumStates=1,ErrStruc='DE',EstCovar=FALSE,Covars=NULL,indivCovar=FALSE,Dmat=NULL,Dfac=NULL,Rfac=NULL,logsdObs=NULL,logsdObsFac=NULL,cholCorr=NULL,cholFac=NULL,MC=FALSE){ 
	#NumStates=1;ErrStruc='DE';EstCovar=FALSE;Covars=NULL;indivCovar=FALSE;Dmat=NULL;Dfac=NULL;Rfac=NULL;logsdObs=NULL;logsdObsFac=NULL;cholCorr=NULL;cholFac=NULL
	#obs<-ZL_OA3
	obs<-t(obs)
	prePars<-setParametersAndFactors(obs=obs,NumStates=NumStates,ErrStruc=ErrStruc,EstCovar=EstCovar,Covars=Covars,indivCovar=indivCovar,Dmat=Dmat,Dfac=Dfac,Rfac=Rfac,logsdObs=logsdObs,logsdObsFac=logsdObsFac,cholCorr=cholCorr,cholFac=cholFac,MC=MC)
	...
	if(MC){
		...
		for(i in 1:50){
			prePars<-setParametersAndFactors(obs=obs,NumStates=NumStates,ErrStruc=ErrStruc,EstCovar=EstCovar,Covars=Covars,indivCovar=indivCovar,Dmat=Dmat,Dfac=Dfac,Rfac=Rfac,logsdObs=logsdObs,logsdObsFac=logsdObsFac,cholCorr=cholCorr,cholFac=cholFac,MC=MC)
			#parSpace[[i]]<-prePars
			obj2<-MakeADFun(data=prePars$data,parameters=prePars$parameters,random="u",
							DLL="dfa1tmb",silent=T,map=list(Z=prePars$Zfac,D=prePars$Dfac,cholCorr=prePars$cholFac,logsdObs=prePars$logsdObsFac,covState=covStateFac))
			opt2<-nlminb(obj2$par,obj2$fn,obj2$gr,control=list(iter.max=200,eval.max=200))
			if(opt2$objective<val | val==1E10){
				obj1<-obj2
				val<-opt2$objective
			}
		}
		...
		obj1 <- MakeADFun(data=prePars$data,parameters=prePars$parameters,random="u",
		DLL="dfa1tmb",silent=T,map=list(Z=prePars$Zfac,D=prePars$Dfac,cholCorr=prePars$cholFac,logsdObs=prePars$logsdObsFac,covState=covStateFac))
		opt1 <- nlminb(obj1$par,obj1$fn,obj1$gr,control=list(iter.max=2000,eval.max=2000))
	}
	...
	for(i in 1:NumStates){
		...
		ScaleFac<-as.vector(apply(pl1$u,2,FUN=sd))#function(x) max(Mod(x))/3))
		pl1$u<-pl1$u/ScaleFac
	...
	pl1$R <- matrix(sdr$value[which(names(sdr$value)=='FullCovMat')],nrow=length(pl1$logsdObs),ncol=length(pl1$logsdObs))
	#pl1$R<- diag(exp(pl1$logsdObs)) %*% obj1$report()$FullCorrMat  %*% diag(exp(pl1$logsdObs))
	#pl1$R<-pl1$cholObs %*% t(pl1$cholObs)
	...
	SES <- list(D=sdr$sd[which(names(sdr$value)=='D')],
			...
			R=matrix(sdr$sd[which(names(sdr$value)=='FullCovMat')],nrow=length(pl1$logsdObs),ncol=length(pl1$logsdObs)))
	
	#Compute AIC.
	AIC<-2*length(opt1$par) + 2*opt1$value;AIC
	
	#print(AIC)
	return(list(Optimization = opt1, Estimates = pl1, Fits = FitSeries,AIC=AIC,StdErr=SES,ParCorrs=sdr$cov.fixed))
}
#
